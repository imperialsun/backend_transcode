...
func TestDemeterChatCompletionsRoute_IsNotRegistered(t *testing.T) {
	app, token, _ := setupDemeterRoutesApp(t, []store.UserPermissionOverrideInput{
		...
		t.Fatalf("chat completion proxy should not reach upstream")
	}))
	...
	if resp.StatusCode != fiber.StatusNotFound {
		t.Fatalf("expected 404 for removed chat completions route, got %d", resp.StatusCode)
	}
}
